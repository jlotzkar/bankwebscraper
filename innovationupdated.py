...
#comments below were pre-figuring out BeautifulSoup looping
...
urlScotia = 'https://www.scotiabank.com/ca/en/about/news/press-releases.html' #will need to use Selenium to toggle 'Entries per page'
...
keywords = ['new', 'technology', 'innovation', 'innovative', 'innovates', 'innovating', 'digital', 'crypto', 'blockchain', 'unveil', 'unveils', 'unveiling', 'launch', 'launches', 'launching', 'announce', 'announces', 'announcing', 'announcement', 'introduce', 'introduces', 'introducing', 'introduction']
...
headers = {
    ...
    'authority': 'scotiabank.investorroom.com',
    ...
    'origin': 'https://www.scotiabank.com',
    ...
    'referer': 'https://www.scotiabank.com/',
...
response = requests.get('https://scotiabank.investorroom.com/api/newsfeed_releases/list.php', headers=headers, params=params)
...
#keywords to search article titles
...
for x in range(0, 200, 50):
    ...
    for word in keywords:
       ...
       response = requests.get('https://scotiabank.investorroom.com/api/newsfeed_releases/list.php', headers=headers, params=params)
        ...
        dataJSON = results['release']

        dataDF = pd.DataFrame(dataJSON)
...
dfScotia.rename(columns={dfScotia.columns[3]: 'Date'}, inplace = True)
...
dfScotia = dfScotia.drop(['id', 'released', 'modified', 'modifiedDate'], axis=1)
...
#only store 100 most recent articles in dataframe

dfScotia = dfScotia.head(100)

 

#changing column name of dataframe

dfScotia = dfScotia.rename(columns={"headline": "Title"})
...
all_dfs = [dfCIBC, dfTD, dfRBC, dfBMO, dfNB]
